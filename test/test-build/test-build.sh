...
if find "$APP_DIR" -name "*.template" | grep -q .; then
...
# 3. ROM is present
if [ -f "$APP_DIR/Contents/Resources/Roms/Halo2600.a26" ]; then
...
if grep -q "Halo 2600 Test" "$APP_DIR/Contents/Info.plist" 2>/dev/null; then
...
if [ -x "$APP_DIR/Contents/MacOS/launch" ]; then
...
if grep -q "Halo2600.a26" "$APP_DIR/Contents/MacOS/launch" 2>/dev/null; then
