...
#
EMU_EMULATOR_PATH="/Applications/ares.app"
if [ ! -d "$EMU_EMULATOR_PATH" ]; then
  bundleError 'Could not find ares (at /Applications/ares.app). Please download it from https://ares-emu.net/download and install it.'
  exit 1
fi
...
if [ "${BUILD_BUNDLED_CONFIG_ENABLED:-true}" = true ]; then
  BUILD_CONFIG_SRC="$HOME/Library/Application Support/ares"
